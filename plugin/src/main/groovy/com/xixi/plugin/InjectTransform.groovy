...
//        Logger.info("flavorAndBuildType ${flavorAndBuildType}")
        ...
        inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                Log.info("项目jar->destName：${destName}");
                /** 重名名输出文件,因为可能同名,会覆盖*/
                ...
                Log.info("====start modifying ${(isJarNeedModify(jarInput.file)).toString()}====");
...
//        if (!"MiPush_SDK_Client_3_2_2.jar".equals(jarFile.name)) {
//            return modified;
//        }
