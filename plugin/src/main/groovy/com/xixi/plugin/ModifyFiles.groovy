    ...
    static File modifyJar(File jarFile, Map<String, Object> modifyMatchMaps, File tempDir, boolean nameHex) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName.endsWith(".class")) {
                String className2
                String className3
                if (entryName.contains("com/xiaomi/mipush/sdk/u")){
                    className2 = entryName.replace("/", ".").replace(".class", "")
                    className3 = Util.path2Classname(entryName)
                    Log.info("====start hasMoreElements${className2} ====");
                    Log.info("====start hasMoreElements${className3} ====");
                    Log.info("====start hasMoreElements${(className2.compareTo(className3)) + ''} ====");
                }
//                className = Util.path2Classname(entryName)
                ...
                if (modifyMatchMaps != null && key != null) {
                    Log.info("====start hasMoreElements${className + key} ====");
